...
functions{
  ...
  /*Hazard function */
  real ht(real T, real sex, real eduyr, real muu1, real muu2, vector mu1, vector mu2, real fshift, real rshift, real b1, real b2, real shape, vector alpha){
    real hr; // hazard function
    real ycog1;
    real ycog2;
    
    ycog1 = YCOG(T, sex, eduyr, muu1, mu1, fshift, rshift, b1);
    ycog2 = YCOG(T, sex, eduyr, muu2, mu2, fshift, rshift, b2);
    hr=shape*pow(T, (shape-1))*exp(alpha[1] + alpha[2]*sex + alpha[3]*eduyr + alpha[4]*ycog1 + alpha[5]*ycog2);
    return(hr);
  }
  
  
   /* Cumulative hazard function */
  real cht(real T, real sex, real eduyr, real muu1, real muu2, vector mu1, vector mu2, real fshift, real rshift, real b1, real b2, real shape, vector alpha,
           vector points, vector weights, int order) {

    vector[order] time;     /** times for the Gauss-Legendre quadrature */
    vector[order] hr;        /** hazard */
    real H; // cumulative hazard
    
    time = T*(points+1)/2;
    
    for (i in 1:order){
    hr[i] = ht(time[i], sex, eduyr, muu1, muu2, mu1, mu2, fshift, rshift, b1, b2, shape, alpha);
    }
    
    H = T/2*sum(weights .* hr);
    return H;
  }
...
parameters{
  ...
  vector[3] mu1; ### add edu year covariate
  real<lower=-1, upper=0> muu2;
  vector[3] mu2; ### add edu year covariate
  real fshift;
  
  real<lower=0> shape;
  vector[5] alpha; ### add ycog2
...
model{
  ...
  for(i in 1:N){
    ...
    if(ddelta[i]==1){//density of survival process in case of death
      target+=log(ht(TIMES[i], sex[i], eduyr[i], muu1, muu2, mu1, mu2, fshift, rshift[i,1], b[i,1], b[i,2], shape, alpha)) - cht(TIMES[i], sex[i], eduyr[i], muu1, muu2, mu1, mu2, fshift, rshift[i,1], b[i,1], b[i,2], shape, alpha, points, weights, order);
    }

    if(ddelta[i]==0){//density of survival process in case of censoring
      target+=-cht(TIMES[i], sex[i], eduyr[i], muu1, muu2, mu1, mu2, fshift, rshift[i,1], b[i,1], b[i,2], shape, alpha, points, weights, order);
  ...
  w=0;
    ...
    for(t in 1:nb_time[i]){
      ...
      sumnormloglik = sumnormloglik + normloglik1 + normloglik2;
		...
		if(ddelta[i]==1){//density of survival process in case of death
      log_lik[i] = sumnormloglik + log(ht(TIMES[i], sex[i], eduyr[i], muu1, muu2, mu1, mu2, fshift, rshift[i,1], b[i,1], b[i,2], shape, alpha)) - cht(TIMES[i], sex[i], eduyr[i], muu1, muu2, mu1, mu2, fshift, rshift[i,1], b[i,1], b[i,2], shape, alpha, points, weights, order);
    }

    if(ddelta[i]==0){//density of survival process in case of censoring
      log_lik[i] = sumnormloglik -cht(TIMES[i], sex[i], eduyr[i], muu1, muu2, mu1, mu2, fshift, rshift[i,1], b[i,1], b[i,2], shape, alpha, points, weights, order);
    }
