...
functions{
  ...
  /*Hazard function */
  real ht(real T, real sex, real eduyr, real rshift, real b1, real b2, real shape, vector alpha){
    real hr; // hazard function
    
    hr=shape*pow(T, (shape-1))*exp(alpha[1] + alpha[2]*sex + alpha[3]*eduyr + alpha[4]*rshift + alpha[5]*b1 + alpha[6]*b2); //hazard function depending on random effects
    return(hr);
  }
  
  
   /* Cumulative hazard function */
  real cht(real T, real sex, real eduyr, real rshift, real b1, real b2, real shape, vector alpha) {

    real H; // cumulative hazard

    H = pow(T, shape)*exp(alpha[1] + alpha[2]*sex + alpha[3]*eduyr + alpha[4]*rshift + alpha[5]*b1 + alpha[6]*b2);
    return H;
  }
...
parameters{
  ...
  vector[3] mu1; // add parameter for eduyr
  real<lower=-1, upper=0> muu2;
  vector[3] mu2; // add parameter for eduyr
  real fshift;
  
  real<lower=0> shape;
  vector[6] alpha; // add parameter for b2
...
model{
  ...
  for(i in 1:N){
    ...
    if(ddelta[i]==1){//density of survival process in case of death
      target+=log(ht(TIMES[i], sex[i], eduyr[i], rshift[i,1], b[i,1], b[i,2], shape, alpha)) - cht(TIMES[i], sex[i], eduyr[i], rshift[i,1], b[i,1], b[i,2], shape, alpha);
    }

    if(ddelta[i]==0){//density of survival process in case of censoring
      target+=-cht(TIMES[i], sex[i], eduyr[i], rshift[i,1], b[i,1], b[i,2], shape, alpha);
  ...
  w=0;
    ...
    for(t in 1:nb_time[i]){
      ...
      sumnormloglik = sumnormloglik + normloglik1 + normloglik2;
		...
		if(ddelta[i]==1){//density of survival process in case of death
      log_lik[i] = sumnormloglik + log(ht(TIMES[i], sex[i], eduyr[i], rshift[i,1], b[i,1], b[i,2], shape, alpha)) - cht(TIMES[i], sex[i], eduyr[i], rshift[i,1], b[i,1], b[i,2], shape, alpha);
    }

    if(ddelta[i]==0){//density of survival process in case of censoring
      log_lik[i] = sumnormloglik -cht(TIMES[i], sex[i], eduyr[i], rshift[i,1], b[i,1], b[i,2], shape, alpha);
    }
