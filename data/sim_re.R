...
sim_re <- function(N, timeinter){
  ...
  b.cov <- matrix(c(0.45^2, 0.45*0.5*0.6, 0.45*0.5*0.6, 0.5^2), ncol = 2) #### corr may need to change
  ...
  alpha3 = -0.05 ### eduyr
  alpha4 = 0.15 ### shift
  alpha5 = 0.02 ### random effect 1
  alpha6 = -0.12 ### random effect 2
  ...
  for (k in 1:n){
    ...
    rate1 = exp(alpha1 + alpha2*unique(longi.y[ID==j]$sex)[1] + alpha3*unique(longi.y[ID==j]$eduyr)[1] + 
                  alpha4*unique(longi.y[ID==j]$rshift)[1] + alpha5*unique(longi.y[ID==j]$b0)[1] + alpha6*unique(longi.y[ID==j]$b0)[2]) # both random intercepts are included
    t1<- rweibull(1, shape = shape1, scale = 1/(rate1^(1/shape1)))
